...
public record TalkSessionCreated(ConferenceId ConferenceId, TalkName Name, TalkDescription Description)
    : IEventPayload<TalkSession, TalkSessionCreated>
{
    public static TalkSession OnEvent(TalkSession aggregatePayload, Event<TalkSessionCreated> ev) =>
        aggregatePayload with { ConferenceId = ev.Payload.ConferenceId, Name = ev.Payload.Name, Description = ev.Payload.Description };
}
