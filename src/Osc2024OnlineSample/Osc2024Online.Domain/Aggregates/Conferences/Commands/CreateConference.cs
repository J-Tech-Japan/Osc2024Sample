...
public record CreateConference(ConferenceId Id, ConferenceName Name, ConferenceDates.IConferenceDates Dates, ConferenceLocation Location)
    : ICommand<Conference>
{
    public Guid GetAggregateId() => Id.Value;
    public class Handler : ICommandHandler<Conference, CreateConference>
    {
        public IEnumerable<IEventPayloadApplicableTo<Conference>> HandleCommand(CreateConference command, ICommandContext<Conference> context)
        {
